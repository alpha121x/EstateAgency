...
if (isset($_POST["add-testimonial"])) {
    ...
    $check = getimagesize($_FILES["testinomial_image"]["tmp_name"]);
    if ($check === false) {
    ...
    if ($_FILES["testinomial_image"]["size"] > 500000) {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["testinomial_image"]["tmp_name"], $targetFile)) {
            // Insert the testimonial data into the database
            $insertQuery = "INSERT INTO testimonials (name, description, image) VALUES ('$testimonialName', '$testimonialAbout', '$targetFile')";
